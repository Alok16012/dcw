...
function LeadFlow({lead,vertical,go,close,notify}){const applying=lead.mode==='apply';const [course,setCourse]=useState(lead.course||'');const [step,setStep]=useState(0),[name,setName]=useState(''),[phone,setPhone]=useState(''),[qualification,setQualification]=useState('12th pass / appearing'),[otp,setOtp]=useState(''),[busy,setBusy]=useState(false),[error,setError]=useState(''),[demoCode,setDemoCode]=useState(''),[result,setResult]=useState(null),[consent,setConsent]=useState(false),[waOptIn,setWaOptIn]=useState(false);
...
const submit=async(attach=resume)=>{let resumeId=attach?.id??null;
  ...
  const d=await api('/api/applications',{kind:isJob?'job':'course',vertical,name:name.trim(),phone,
    ...
    where:lead.where||null,course:course||undefined,resumeId,
  ...
  const record={id:d.application.id,name:name.trim(),phone,qualification,interest:lead.interest,course:course||null,kind:'Application',title:lead.title,createdAt:new Date().toISOString(),status:d.application.status};
