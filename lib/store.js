 ...
 */
import { boards, countries, skillCourses } from './data/index.js';
...
export function listEntities(vertical, opts = {}) {
  ...
  let rows = vertical === 'jobs'
    ...
    : allInstitutions().filter(i => i.isActive && i.vertical === vertical).map(institutionCard);
...
export function predictColleges({ rank, category = 'Gen', budget, exam = 'NEET-UG' }) {
  ...
  for (const inst of allInstitutions().filter(i => i.vertical === 'colleges' && i.isActive)) {
