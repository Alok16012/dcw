/**
 * In-memory query layer over the demo dataset.
 *
 * This is the single source of truth for both the React UI and the /api routes,
 * so the two can never drift. Swapping in Supabase means reimplementing these
 * functions against the real tables — the call sites do not change.
 */
import { countries, skillCourses } from './data/index.js';
// Boards are editable from /admin/boards, so they come from their repository for
// the same reason jobs and institutions do.
import { listBoards } from './boards-repo.js';
// Jobs, companies, institutions and courses are all mutable (admin CRUD), so
// they come from their repositories rather than the frozen seeds — a job posted
// or a fee corrected in /admin reaches the public listing at once.
import { listJobs, findJob, findCompany, listCompanies } from './jobs-repo.js';
import { allInstitutions, isPublic } from './institutions-repo.js';
// Reviews written on DCW are keyed to the institution they are about, so a card
// can say how many of them there are to read. They are kept as their own pair of
// fields rather than folded into `rating`/`reviews`: the catalogue figure is the
// institution's overall standing, the DCW figure is what somebody can actually
// open and read here, and blending the two would misstate both.
import { summaryByInstitution, summariseReviews } from './reviews-repo.js';
// A card leads with a picture. The institution's own photograph when the console
// holds one, an illustrative scene labelled as such when it does not — the rule
// and the reasoning both live in lib/photos.js.
import { institutionPhoto } from './photos.js';

const money = n => '₹' + n.toLocaleString('en-IN');
const yearsFromMonths = m => (m % 12 === 0 ? `${m / 12} years` : `${(m / 12).toFixed(1)} years`);

function approvalLabel(a) {
  if (a.body === 'UGC-DEB') return 'UGC entitled';
  if (a.body === 'INI') return 'INI';
  if (a.body === 'NMC') return 'NMC approved';
  return `${a.body} ${a.grade}`;
}

/**
 * Projects an Institution into the flat shape the listing/detail cards render.
 *
 * @param {Object} inst
 * @param {Map<string,{count:number,average:number}>} [summaries] review counts,
 *   precomputed once per list so a page of 20 cards is one pass over the reviews
 *   rather than twenty.
 */
export function institutionCard(inst, summaries) {
  const c = inst.courses[0] ?? null;
  const rv = (summaries ?? summaryByInstitution()).get(inst.id) ?? null;
  const topCutoff = inst.cutoffs.find(x => x.category === 'Gen');
  // The entrance exam is a property of the course, not of the vertical. Calling
  // every college cutoff a NEET cutoff labelled NIT Patna — a JEE Main
  // institute — as a medical seat, so read the exam the course actually takes.
  const exam = c?.examAccepted?.[0] ?? null;
  // Per-category closing ranks, keyed the way the seed keys them ('Gen',
  // 'OBC', …), so consumers can use a real cutoff instead of guessing an offset.
  const cutoffs = Object.fromEntries(inst.cutoffs.map(x => [x.category, x.closingRank]));
  const photo = institutionPhoto(inst);
  return {
    id: inst.id, slug: inst.slug, vertical: inst.vertical, mark: inst.mark, name: inst.name,
    /* The card's picture. `image` is the card-sized file and `imageFull` the
       one the detail hero stretches across the page; they are the same URL when
       the console holds a real photograph, because an uploaded file has only
       one size. `imageIllustrative` is the field every surface must honour: it
       is true when the picture is a library scene standing in for a campus, and
       the card then has to say so rather than let the reader assume they are
       looking at the place they are about to apply to. The monogram stays on
       the plate either way — it is the mark that identifies the institution. */
    image: photo.src, imageFull: photo.srcFull, imageAlt: photo.alt,
    imageIllustrative: photo.illustrative,
    place: `${inst.city}, ${inst.state}`, type: inst.type,
    // Two facts the card projection dropped, both of which the public listing
    // now filters on. `streams` is the set of subjects this institution
    // actually teaches — the seed records it per course, so a "Medical" or
    // "Engineering" filter can be answered from the catalogue instead of being
    // guessed from the name. `country` is what makes a study-abroad filter
    // possible at all. Both are additive: nothing that read this shape before
    // sees a changed field.
    streams: [...new Set(inst.courses.map(x => x.stream).filter(Boolean))],
    country: inst.country ?? null,
    course: c?.name ?? '—',
    fee: c?.totalFee ?? 0,
    emi: inst.vertical === 'colleges'
      ? (c?.seats != null ? `${c.seats} seats` : '—')
      : (c?.emiMonthly != null ? `${money(c.emiMonthly)}/mo` : 'No EMI'),
    rating: inst.rating, reviews: inst.reviews,
    // Reviews written on DCW about this institution — the ones a visitor can
    // click through and read. Zero is a truthful answer and the UI says so.
    dcwReviews: rv?.count ?? 0, dcwRating: rv?.average ?? null,
    // Google's own rating, when the listing has been mapped from the admin
    // console. Never averaged into the figures above: it is Google's number and
    // is shown under Google's name.
    google: inst.google ?? null,
    // The one-line, plain-language answer to "what is this place". Requirement 3.
    plainSummary: inst.plainSummary ?? null,
    highlights: inst.highlights ?? [],
    proofCount: inst.proofOfWork?.length ?? 0,
    approval: inst.approvals.map(approvalLabel).slice(0, 3),
    // The strike-through price is real catalogue data, not a display flourish:
    // it is the fee before the current intake discount, so the card must carry
    // it rather than reconstruct one.
    mrp: c?.mrpFee ?? null,
    duration: c ? yearsFromMonths(c.durationMonths) : '—',
    exam, cutoffs, cutoff: topCutoff?.closingRank ?? null,
    mode: inst.vertical === 'colleges' && topCutoff
      ? `${exam ?? 'Entrance'} • cutoff ${topCutoff.closingRank}`
      : (c?.mode ?? '—'),
    deadline: c?.deadline ?? '—',
    /* The menu somebody applying has to choose from. It used to live in a
       second hardcoded map keyed by institution id, which meant a fee could be
       corrected in the catalogue and stay wrong in the apply dialog. The card
       now carries the catalogue's own rows, projected into the shape the apply
       flow and the /distance path filters already speak — lowercase level and
       mode, because those are the two questions those filters ask. */
    courses: inst.courses.filter(x => x.isActive !== false).map(courseRow),
    featured: inst.featuredPriority >= 80, priority: inst.featuredPriority
  };
}

/** One InstitutionCourse, in the flat shape the UI renders and filters on. */
function courseRow(c) {
  return {
    name: c.name,
    duration: yearsFromMonths(c.durationMonths),
    fee: c.totalFee, mrp: c.mrpFee ?? null, note: c.note ?? null,
    level: c.level === 'UG' ? 'ug' : c.level === 'PG' ? 'pg' : null,
    mode: /online/i.test(c.mode) ? 'online' : /distance|open school/i.test(c.mode) ? 'distance' : null
  };
}

/** Projects a Job into the same flat card shape, so one card component serves all verticals. */
/** Whole days since an ISO date, used for the "posted this week" facet. */
function daysSince(iso) {
  const d = Math.floor((Date.now() - new Date(iso + 'T00:00:00').getTime()) / 86400000);
  return Number.isFinite(d) ? Math.max(0, d) : null;
}

const monthly = n => '₹' + Math.round(n / 12).toLocaleString('en-IN');

/** Projects a Job into the same flat card shape, so one card component serves all verticals. */
export function jobCard(job) {
  const co = findCompany(job.companyId);
  const pay = job.salaryMin === job.salaryMax
    ? `${monthly(job.salaryMin)}/mo`
    : `${monthly(job.salaryMin)}–${monthly(job.salaryMax).replace('₹', '')}/mo`;
  const city = job.city ?? (job.wfh ? 'Remote' : job.location.split(',').slice(-2, -1)[0]?.trim() ?? job.location);
  return {
    // The public URL is /jobs/:slug, and saved/compare entries are keyed on this
    // id, so the card id is the slug rather than the internal record id.
    id: job.slug, slug: job.slug, vertical: 'jobs', mark: co?.mark ?? 'JB', name: job.title,
    company: co?.name ?? 'Company',
    // The id as well as the name: reviews are keyed to the employer, so the
    // detail page needs something to filter on that is not a display string.
    companyId: job.companyId ?? null,
    place: `${co?.name ?? 'Company'} • ${job.wfh ? 'Work from home' : (job.area ? `${job.area}, ${city}` : city)}`,
    city, area: job.area ?? null, wfh: !!job.wfh,
    // Coordinates travel with the card because "jobs near me" ranks on them.
    pos: job.lat != null && job.lng != null ? [job.lat, job.lng] : null,
    type: job.jobType,
    course: job.eligibility ?? `${job.qualification} • ${job.experienceMin === 0 ? 'Fresher' : job.experienceMin + '+ yrs'}`,
    qualification: job.qualification, experienceMin: job.experienceMin,
    fee: job.salaryMin, salaryMax: job.salaryMax,
    emi: `${job.openings} opening${job.openings === 1 ? '' : 's'}`,
    // The formatted string above is for a card; the raw count is for anything
    // that has to add openings up (the hero proof tiles total them).
    openings: job.openings,
    // Employer reputation belongs to the company. The previous projection
    // invented reviews as openings × 8, which read as data but was arithmetic.
    rating: co?.rating ?? null, reviews: co?.reviews ?? null,
    approval: job.tags ?? [co?.isVerified ? 'Verified company' : 'Unverified', job.wfh ? 'Work from home' : null].filter(Boolean),
    duration: job.jobType === 'Internship' ? `${pay} stipend` : pay,
    mode: job.roleType ?? job.industry,
    sector: job.industry,
    postedDays: daysSince(job.postedOn),
    deadline: job.deadlineLabel
      ?? `Apply by ${new Date(job.expiresOn).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}`,
    featured: job.featured, priority: job.featured ? 90 : 50
  };
}

const SORTS = {
  relevance: (a, b) => b.featured - a.featured || b.priority - a.priority || (b.rating ?? 0) - (a.rating ?? 0),
  'fee-asc': (a, b) => a.fee - b.fee,
  'fee-desc': (a, b) => b.fee - a.fee,
  rating: (a, b) => (b.rating ?? 0) - (a.rating ?? 0),
  fastest: (a, b) => parseFloat(a.duration) - parseFloat(b.duration)
};

/**
 * @param {'distance'|'colleges'|'jobs'} vertical
 * @param {{q?:string,type?:string,mode?:string,feeMin?:number,feeMax?:number,approval?:string,
 *          wfh?:boolean,qualification?:string,city?:string,sector?:string,
 *          sort?:string,page?:number,pageSize?:number}} opts
 */
export function listEntities(vertical, opts = {}) {
  const { q, type, mode, feeMin, feeMax, approval, wfh, qualification, city, sector,
    sort = 'relevance', page = 1, pageSize = 20 } = opts;

  // `isPublic` is deliberately the repository's own predicate rather than a
  // local `isActive` test: an institution added through the admin console starts
  // as a draft so it can be reviewed before it goes live, and a draft that leaks
  // onto the public listing defeats the point of the review step.
  const summaries = summaryByInstitution();
  let rows = vertical === 'jobs'
    ? listJobs().map(jobCard)
    : allInstitutions().filter(i => isPublic(i) && i.vertical === vertical)
      .map(i => institutionCard(i, summaries));

  if (q) {
    const needle = q.toLowerCase();
    rows = rows.filter(r => `${r.name} ${r.place} ${r.course} ${r.mode}`.toLowerCase().includes(needle));
  }
  if (type) rows = rows.filter(r => r.type.toLowerCase() === type.toLowerCase());
  if (mode) rows = rows.filter(r => r.mode.toLowerCase().includes(mode.toLowerCase()));
  if (feeMin != null) rows = rows.filter(r => r.fee >= feeMin);
  if (feeMax != null) rows = rows.filter(r => r.fee <= feeMax);
  if (approval) rows = rows.filter(r => r.approval.some(a => a.toLowerCase().includes(approval.toLowerCase())));
  // Both of these used to re-derive from the source rows: wfh by rebuilding an
  // id set, qualification by string-matching the display line. The card now
  // carries the real fields, so they filter on the value rather than its prose.
  if (wfh != null && vertical === 'jobs') rows = rows.filter(r => r.wfh === wfh);
  if (qualification && vertical === 'jobs') {
    rows = rows.filter(r => r.qualification?.toLowerCase() === qualification.toLowerCase());
  }
  if (city && vertical === 'jobs') rows = rows.filter(r => r.city === city);
  if (sector && vertical === 'jobs') rows = rows.filter(r => r.sector === sector);

  rows.sort(SORTS[sort] ?? SORTS.relevance);

  const total = rows.length;
  const start = (page - 1) * pageSize;
  return {
    rows: rows.slice(start, start + pageSize),
    page, pageSize, total, totalPages: Math.max(1, Math.ceil(total / pageSize)),
    hasMore: start + pageSize < total
  };
}

/**
 * One institution, in the shape the detail page renders.
 *
 * `includeDrafts` exists for exactly one caller: the admin preview, which has to
 * show an editor the page they are about to publish. Public routes leave it off
 * and an unpublished listing is a 404 to them.
 */
export function getInstitution(vertical, slug, { includeDrafts = false } = {}) {
  const inst = allInstitutions().find(i => i.vertical === vertical && (i.slug === slug || i.id === slug));
  if (!inst || (!includeDrafts && !isPublic(inst))) return null;
  return { ...inst, card: institutionCard(inst), reviewSummary: summariseReviews({ institutionId: inst.id }) };
}

export function getJob(idOrSlug) {
  const job = findJob(idOrSlug);
  if (!job) return null;
  return { ...job, company: findCompany(job.companyId), card: jobCard(job) };
}

/* ── RECOGNITION & APPROVAL, AND PROOF OF WORK ──────────────────────────────
   Both sections appear on all three vertical homes, and neither is authored.
   They are counted out of the same records the detail pages already list and
   the console already edits — an approval added in /admin/catalogue, or a
   result sheet uploaded in /admin/jobs, changes these numbers on the next
   request. That is the point of computing them rather than writing them down:
   a hand-written "UGC-approved, 20+ universities" on a homepage is a claim
   nobody updates, and it goes stale the first time the catalogue moves. */

/**
 * Which of a job's tags are a credential rather than a perk.
 *
 * A tag is a free string the console writes, so a section headed "Recognition
 * and approval" cannot simply list all of them — "Cab facility" is a nice thing
 * about a job, not a check anybody carried out. Keyed lowercase because the
 * seed spells the statutory one both ways round.
 */
const JOB_CREDENTIALS = {
  'verified company': 'Employer verified',
  'salary verified': 'Salary verified',
  'pf + esi': 'PF + ESI declared',
  'esi + pf': 'PF + ESI declared'
};

/** Newest evidence first. A year is coarser than a timestamp, so it leads. */
const byRecency = (a, b) => (b.year ?? 0) - (a.year ?? 0)
  || String(b.addedAt ?? '').localeCompare(String(a.addedAt ?? ''));

function group(map, key, seed) {
  const g = map.get(key) ?? { key, count: 0, verified: 0, documents: 0, expires: null, examples: [], ...seed };
  map.set(key, g);
  return g;
}

function institutionCredentials(vertical) {
  const rows = allInstitutions().filter(i => isPublic(i) && i.vertical === vertical);
  const groups = new Map();
  for (const inst of rows) {
    for (const a of inst.approvals) {
      if (!a.body) continue;
      /* Keyed on the display label, not on body+grade. `approvalLabel` already
         collapses the bodies whose grade is not a grade — NMC "Approved" and
         NMC "Listed" are both "NMC approved" to a reader — and keying on the
         pair produced two adjacent cards with the same words on them. */
      const label = approvalLabel(a);
      /* `filter` is what the Recognition chip puts in the query string, and it is
         the label rather than the body code on purpose: the listing filters on
         `card.approval`, which holds these same labels, so a chip carrying
         "UGC-DEB" would land on an empty result page. */
      const g = group(groups, label, { body: a.body, grade: a.grade || null, label, filter: label });
      g.count++;
      if (a.verified) g.verified++;
      if (a.documentId || a.certificateUrl) g.documents++;
      // The soonest expiry in the group, because that is the one that matters:
      // it is the date by which somebody has to go and ask for a new letter.
      if (a.validTill && (!g.expires || a.validTill < g.expires)) g.expires = a.validTill;
      if (g.examples.length < 3) g.examples.push(inst.name);
    }
  }
  const proof = rows.flatMap(i => i.proofOfWork.map(p => ({
    ...p, owner: i.name, ownerMark: i.mark, href: `/${vertical}/${i.slug}#proof`
  })));
  return {
    vertical,
    listings: rows.length,
    covered: rows.filter(i => i.approvals.length > 0).length,
    approvals: [...groups.values()].sort((a, b) => b.count - a.count).slice(0, 6),
    proof: proof.sort(byRecency).slice(0, 6),
    proofTotal: proof.length
  };
}

function jobCredentials() {
  const live = listJobs();
  const groups = new Map();
  for (const j of live) {
    const co = findCompany(j.companyId);
    for (const tag of j.tags ?? []) {
      const label = JOB_CREDENTIALS[tag.toLowerCase()];
      if (!label) continue;
      /* The tag as written, not the tidied label: a job card's `approval` is its
         raw tag list, so "Employer verified" would filter to nothing while
         "Verified company" — the tag itself — finds every role that carries it. */
      const g = group(groups, label, { body: label, grade: null, label, filter: tag, owners: new Set() });
      g.count++;
      // For an employer, "verified" is a property of the company rather than of
      // the posting, so the same role can carry a credential the company has
      // not been checked for. Counting both keeps that visible.
      if (co?.isVerified) g.verified++;
      if (co) g.owners.add(co.id);
      if (co && g.examples.length < 3 && !g.examples.includes(co.name)) g.examples.push(co.name);
    }
  }
  /* Evidence belongs to the employer, but only employers who are actually
     hiring should appear on a jobs home — and a company has no page of its own,
     so each entry links to one of that employer's live roles, where the same
     Proof of Work section already renders. */
  const firstRole = new Map();
  for (const j of live) if (!firstRole.has(j.companyId)) firstRole.set(j.companyId, j.slug);
  const proof = listCompanies().filter(c => firstRole.has(c.id)).flatMap(c => c.proofOfWork.map(p => ({
    ...p, owner: c.name, ownerMark: c.mark, href: `/jobs/${firstRole.get(c.id)}#proof`
  })));
  return {
    vertical: 'jobs',
    listings: live.length,
    covered: live.filter(j => (j.tags ?? []).some(t => JOB_CREDENTIALS[t.toLowerCase()])).length,
    approvals: [...groups.values()].sort((a, b) => b.count - a.count).slice(0, 6)
      // A Set does not survive JSON, and the count is the only part anybody reads.
      .map(g => ({ ...g, owners: g.owners.size })),
    proof: proof.sort(byRecency).slice(0, 6),
    proofTotal: proof.length
  };
}

/** @param {'distance'|'colleges'|'jobs'} vertical */
export function credentials(vertical) {
  return vertical === 'jobs' ? jobCredentials() : institutionCredentials(vertical);
}

/** Compare up to 3 items; flags which rows actually differ (PRD §5.5). */
export function compare(vertical, ids) {
  const picked = ids.slice(0, 3).map(id => vertical === 'jobs'
    ? (getJob(id)?.card ?? null)
    : (getInstitution(vertical, id)?.card ?? null)).filter(Boolean);
  const fields = ['fee', 'duration', 'mode', 'deadline', 'rating', 'emi', 'type'];
  const rows = fields.map(f => {
    const values = picked.map(p => p[f]);
    return { field: f, values, differs: new Set(values.map(String)).size > 1 };
  });
  return { items: picked, rows };
}

export function searchAll(q, vertical) {
  if (!q || q.trim().length < 2) return { query: q ?? '', current: [], other: [] };
  const hit = v => listEntities(v, { q, pageSize: 5 }).rows.map(r => ({ ...r, vertical: v }));
  const current = vertical ? hit(vertical) : [];
  const other = ['distance', 'colleges', 'jobs'].filter(v => v !== vertical).flatMap(hit).slice(0, 6);
  return { query: q, current, other };
}

/** Rank predictor (PRD §6.4). Buckets by margin against last-year closing rank. */
/** The counselling forms say 'General'; the cutoff table says 'Gen'. */
const CATEGORY_KEY = { General: 'Gen', Gen: 'Gen', OBC: 'OBC', SC: 'SC', ST: 'ST', EWS: 'EWS' };

/**
 * Buckets colleges against a rank using their recorded closing ranks.
 *
 * `exam` is not optional decoration: a closing rank only means something
 * alongside the exam that produced it, and the catalogue holds both medical
 * (NEET-UG) and engineering (JEE Main) institutions. Without the filter a NEET
 * rank was being matched against NIT Patna's JEE closing rank.
 *
 * A college with no cutoff for the requested category falls back to the
 * General cutoff; `exactCategory` records which of the two was used so the
 * interface can say so rather than implying a category-specific prediction.
 */
export function predictColleges({ rank, category = 'Gen', budget, exam = 'NEET-UG' }) {
  const key = CATEGORY_KEY[category] ?? 'Gen';
  const out = { strong: [], possible: [], backup: [] };
  for (const inst of allInstitutions().filter(i => i.vertical === 'colleges' && isPublic(i))) {
    if (exam && !inst.courses.some(c => c.examAccepted?.includes(exam))) continue;
    const exact = inst.cutoffs.find(c => c.category === key);
    const cut = exact ?? inst.cutoffs.find(c => c.category === 'Gen');
    if (!cut) continue;
    const card = { ...institutionCard(inst), closingRank: cut.closingRank, cutoffCategory: cut.category, exactCategory: Boolean(exact) };
    if (budget != null && card.fee > budget) continue;
    if (rank <= cut.closingRank * 0.8) out.strong.push(card);
    else if (rank <= cut.closingRank * 1.15) out.possible.push(card);
    else out.backup.push(card);
  }
  return out;
}

export function checkAbroadEligibility({ marks, neetQualified, budget }) {
  const reasons = [];
  if (marks < 50) reasons.push('NMC requires at least 50% in PCB for foreign medical study.');
  if (!neetQualified) reasons.push('NEET qualification is required to practise in India after an overseas MBBS.');
  const affordable = countries.filter(c => budget == null || c.totalCostMin <= budget);
  return { eligible: reasons.length === 0, reasons, countries: affordable };
}

export { countries, skillCourses, listCompanies, listBoards };
