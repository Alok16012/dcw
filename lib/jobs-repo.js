...
import { validateProof, buildProofEntry } from './proof.js';
import { getOptions, isOption } from './option-lists.js';

/* Every company carries a proof list from the moment it exists, so the console
   and the public page never have to guard against a missing array. */
const withProof = c => ({ ...c, proofOfWork: c.proofOfWork ? c.proofOfWork.map(p => ({ ...p })) : [] });

let jobs = seedJobs.map(j => ({ ...j }));
let companies = seedCompanies.map(withProof);
let seq = 0;

/* Getters, not a snapshot: both lists are editable under Dropdowns & lists, and
   a value added there has to be offered by the job form and accepted by the
   validation below in the same breath. Reading at call time is what makes the
   two agree without a deploy. */
export const JOB_ENUMS = {
  get qualifications() { return getOptions('job.qualification'); },
  get jobTypes() { return getOptions('job.type'); }
};

/* What a field falls back to when the form leaves it alone. The literal is the
   intent — most postings are full-time and most ask for nothing in particular —
   but it is checked against the live list first, because renaming `Any` under
   Dropdowns & lists would otherwise have new jobs defaulting to a string the
   dropdown no longer offers and validateJob() no longer accepts. */
const fallback = (key, want) => (isOption(key, want) ? want : (getOptions(key)[0] ?? want));
...
export function validateJob(input, { partial = false } = {}) {
  ...
  if (has('qualification') && !isOption('job.qualification', input.qualification)) {
    e.qualification = `Qualification must be one of: ${getOptions('job.qualification').join(', ')}.`;
  }
  if (has('jobType') && !isOption('job.type', input.jobType)) {
    e.jobType = `Job type must be one of: ${getOptions('job.type').join(', ')}.`;
  }
...
export function createJob(input, { actor = 'admin' } = {}) {
  ...
  const job = {
    ...
    qualification: input.qualification ?? fallback('job.qualification', 'Any'),
    experienceMin: Number(input.experienceMin ?? 0),
    jobType: input.jobType ?? fallback('job.type', 'Full-time'),
...
/**
 * Proof of Work for the jobs vertical (requirement 8).
 *
 * It hangs off the employer rather than the posting, because an offer letter or
 * a photograph from a hiring drive is evidence about the company — filing it
 * per vacancy would mean re-uploading it every time the same employer posts
 * again, and a candidate looking at one of five postings would see proof on
 * some and none on the rest.
 */
export { PROOF_KINDS } from './proof.js';

export function addCompanyProof(companyId, input, { actor = 'admin' } = {}) {
  ...
  const entry = buildProofEntry(company.id, company.proofOfWork.length + 1, input, actor);
  company.proofOfWork.push(entry);
  return { ok: true, proof: entry, company };
}

export function deleteCompanyProof(companyId, proofId) {
  const company = findCompany(companyId);
  if (!company) return { ok: false, error: 'NOT_FOUND' };
  const i = company.proofOfWork.findIndex(p => p.id === proofId);
  if (i === -1) return { ok: false, error: 'NOT_FOUND' };
  const [proof] = company.proofOfWork.splice(i, 1);
  return { ok: true, proof, company };
}
...
export const __resetJobs = () => {
  ...
  companies = seedCompanies.map(withProof);
  seq = 0;
};
