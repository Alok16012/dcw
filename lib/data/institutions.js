...
/* Proof of Work seeds — the same shape lib/proof.js builds, so a seeded record
   and one uploaded in the console render through one card.

   `url` and `documentId` are null on purpose. These stand for documents held at
   the office; the console is where a real file gets attached, and the public
   card says which of the two it is looking at. Inventing a link to a result
   sheet that does not exist is the one thing a Proof of Work section must not
   do — it is the section whose entire job is to be checkable. */
const pow = (owner, n, kind, title, year, summary, extra = {}) => ({
  id: `pow-${owner}-${n}`, kind, title, year, summary,
  courseName: null, documentId: null, documentName: null, mimeType: null, url: null,
  addedBy: 'seed', addedAt: `${year}-07-01T00:00:00.000Z`, ...extra
});
...
export const institutions = [
  {
    ...
    /* A campus address and its coordinates — a mapping, not a claim. `placeId`,
       `rating` and `reviews` are deliberately absent: a place id is an
       identifier we would send to Google, so it has to be looked up in the
       console rather than typed into a seed file, and the rating may only come
       from a live Places call (see lib/integrations/maps.js). Latitude and
       longitude are enough to draw the map and open directions. */
    google: { lat: 28.5449, lng: 77.3327,
    ...
    proofOfWork: [
      pow('amity-online', 1, 'Result', 'Online MBA result sheet, even semester', 2025,
        'Semester result issued by the university for the 2023–25 online MBA cohort.', { courseName: 'Online MBA' }),
      pow('amity-online', 2, 'Admission letter', 'Admission letter, January 2025 intake', 2025,
        'Enrolment confirmation issued to a DCW-referred applicant, personal details removed.')
    ]
  },
  {
    ...
    proofOfWork: [
      pow('manipal-online', 1, 'Certificate', 'UGC-DEB entitlement letter, 2025–27', 2025,
        'The entitlement this listing rests on, held at our Patna office.'),
      pow('manipal-online', 2, 'Placement', 'Placement record, Online BCA 2025', 2025,
        'Offer letters collected from students who finished the 2022–25 online BCA.', { courseName: 'Online BCA' })
    ]
  },
  {
  ...
  },
  {
    ...
    cutoffs: [], placement: null,
    proofOfWork: [
      pow('ignou', 1, 'Result', 'BA (General) TEE result, June 2025 cycle', 2025,
        'Term-end examination result for a Patna-based student we enrolled in 2022.', { courseName: 'BA (General)' }),
      pow('ignou', 2, 'Event', 'Regional centre admission camp, Patna', 2024,
        'Photographs and the attendance sheet from the January 2024 walk-in session.')
    ]
  },
  {
...

  {
    ...
    placement: null,
    proofOfWork: [
      pow('gmc-patna', 1, 'Admission letter', 'Bihar UGMAC allotment letter, 2025', 2025,
        'Seat allotment for a student we counselled through the state round, personal details removed.', { courseName: 'MBBS' }),
      pow('gmc-patna', 2, 'Certificate', 'NMC approval, valid to March 2029', 2025,
        'The approval the seat count on this listing is taken from.')
    ]
  },
  {
  ...
  },
  {
    ...
    proofOfWork: [
      pow('nit-patna', 1, 'Placement', 'B.Tech CSE placement report, 2025', 2025,
        'The institute’s own published placement report, which the averages on this listing are read from.', { courseName: 'B.Tech CSE' }),
      pow('nit-patna', 2, 'Admission letter', 'JoSAA Round 1 allotment, 2025', 2025,
        'Seat allotment for a Patna student we guided through JoSAA choice-filling.')
    ]
  },
  {
    ...
    placement: null,
    proofOfWork: [
      pow('aiims-patna', 1, 'Result', 'MCC Round 1 closing ranks, 2025', 2025,
        'The counselling result the cutoffs on this listing are taken from, category by category.')
    ]
  },
  {
