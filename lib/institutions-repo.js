...
/* The level, mode and stream vocabularies are editable from the console now, so
   validation reads the live list rather than a constant. The arrays below are
   only the fallbacks used when a course row arrives with nothing to go on. */
...
/* Read through getOptions() rather than captured at module load: the console can
   add a level while the process is running, and a consumer holding a snapshot
   taken at boot would reject it. */
export const INSTITUTION_ENUMS = {
  get levels() { return getOptions('course.level'); },
  get modes() { return getOptions('course.mode'); },
  get streams() { return getOptions('course.stream'); },
  get types() { return getOptions('institution.type'); },
  verticals: ['distance', 'colleges'],
  statuses: STATUSES,
  get proofKinds() { return getOptions('proof.kind'); }
};
...
function bootstrapInstitution(inst) {
  ...
  return {
    ...
    /* The campus photograph, and the line describing it. Null on every seeded
       row on purpose: we hold no licensed picture of any of these campuses, and
       the public card is built to say so — lib/photos.js hands it a labelled
       illustrative scene until somebody uploads the real thing here. */
    image: inst.image ?? null,
    imageAlt: inst.imageAlt ?? null
  };
}
...
export function validateCourse(input, { partial = false } = {}) {
  ...
  if (has('level') && !isOption('course.level', input.level)) {
    e.level = `Level must be one of: ${getOptions('course.level').join(', ')}.`;
  }
  if (has('mode') && !isOption('course.mode', input.mode)) {
    e.mode = `Mode must be one of: ${getOptions('course.mode').join(', ')}.`;
  }
...
export function createInstitution(input, { actor = 'admin' } = {}) {
  ...
  const inst = {
    ...
    imageAlt: input.imageAlt ? String(input.imageAlt).trim() : null,
...
export function updateInstitution(id, patch, { actor = 'admin' } = {}) {
  ...
  const writable = ['name', 'vertical', 'type', 'city', 'state', 'country', 'description',
    'plainSummary', 'prospectusUrl', 'image', 'imageAlt', 'mark', 'isActive', ...numeric];
