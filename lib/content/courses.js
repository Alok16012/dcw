...
export function coursesOf(item){
  /* The card now carries the catalogue's own course rows (lib/store.js
     institutionCard), so that is the answer whenever it is present: one edit in
     /admin/catalogue changes the listing, the apply dialog and the instalment
     plan together. COURSES below is the pre-catalogue copy, kept as the seed the
     repository folds in at boot and as the fallback for anything rendered from a
     card shape that predates the field. */
  if(Array.isArray(item.courses)&&item.courses.length)return item.courses;
  return (COURSES[item.id]||[[item.course,item.duration,item.fee,item.mrp??null,item.mode,null,null]]).map(([name,duration,fee,mrp,note,level,mode])=>({name,duration,fee,mrp,note,level:level??null,mode:mode??null}))}
