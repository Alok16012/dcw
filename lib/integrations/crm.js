...
export function upsertLead(input) {
  ...
  if (existing) {
    ...
    activity.push({ leadId: existing.id, at: new Date().toISOString(), type: 'repeat_enquiry',
      note: `Repeat enquiry from ${input.vertical} (${input.interestType}${input.interestId ? ': ' + input.interestId : ''})` });
  ...
  const lead = {
    ...
    name: input.name, phone, whatsappSame: input.whatsappSame !== false,
